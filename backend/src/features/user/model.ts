...
export const createUserModel = async (email: string, password: string, name: string) => {
  try {
    const query = `
      INSERT INTO users
      (email, password, name)
...
// Update user data
export const updateUserModel = async (userId: number, email: string, password: string, name: string) => {
  try {
    const query = `
      UPDATE users
