...
const Task = ({ task, focus = false }: TaskComponentProps) => {
  const { deleteTask, deleteLocalTask, updateTask, uploadLocalTask } = useTask();
  ...
  const handleBlur = (e: React.FocusEvent<HTMLTextAreaElement>) => {
    ...
    if (newTitle === "" && newDescription === "") {
      ...
      if (task.userId) {
        ...
        uploadLocalTask({ title: newTitle, description: newDescription, id: task.id, isDone: false });
      }
    }
  ...
  return (
    <div className={`Task my-1.5 bg-gray-100 hover:bg-gray-200 ${(isFocused || isFocused) && "bg-gray-200"}`}>
      <div className={`relative p-2.5 flex items-start gap-2.5 ${task.isDone ? "opacity-40" : ""}`}>
        ...
        {/* CHECKBOX */}
        <div onClick={() => updateTask(task.id, { isDone: !task.isDone })} className="w-5 h-5 mt-0.5 flex-shrink-0">
